    ...
    try {
      if (type === "my-votes") {
        ...
        const processedVotings = (data.debates ?? []).map((vote) => {
          ...
          const leftPercent =
            totalVotes > 0 ? (vote.leftCount / totalVotes) * 100 : 0;
          const rightPercent =
            totalVotes > 0 ? (vote.rightCount / totalVotes) * 100 : 0;
        ...
        const data: VotingScrapResponse = await getMypageVotingScraps(
          cursorId,
          10
        );
        //console.log(data)
    ...
    <FlatList
      ...
      renderItem={({ item }) => (
        <VotingCard
          ...
          onScrapToggle={() =>
            console.log(`Scrap toggled for vote ${item.debateId}`)
          }
